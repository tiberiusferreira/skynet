...
use std::fs::File;
use skynet::dataset::common_structs::{SimpleBbox, ImgFilenameWithBboxes};
use std::collections::HashMap;
use skynet::dataset::data_transformers::bbox_conversion::{draw_bb_to_img, draw_bb_to_img_from_file};
use imageproc::drawing::Blend;
use image::{GenericImageView, FilterType, DynamicImage, open};
use rayon::prelude::*;
use std::io;
use std::io::Write;
...
pub fn hashmap2img_filename_with_bboxes(hashmap: HashMap<ImgFileData, Vec<SimpleBbox>>) -> Vec<ImgFilenameWithBboxes>{
    hashmap.into_iter().map(|(img_file_data, bb_vec)|{
...
fn main(){
    ...
    io::stdout().flush().unwrap();
    img_filename_with_bboxes_vec.par_iter_mut().for_each(|img_and_bbs|{
        ...
        resized.save(&format!("coco/train/{}", img_and_bbs.img_filename)).expect("error saving img");
...
fn resize_bb(bboxes: &Vec<SimpleBbox>, width_multiplier: f32, height_multiplier: f32) -> Vec<SimpleBbox>{
    bboxes
        .into_iter()
        .map(|obj| SimpleBbox {
